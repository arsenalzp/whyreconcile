...
func (hdlr ExternalResourceHandler) Create(ctx context.Context, e event.TypedCreateEvent[client.Object], q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
...
func (hdlr ExternalResourceHandler) Delete(ctx context.Context, e event.TypedDeleteEvent[client.Object], q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	watch := hdlr.watchName
	...
	cause := causes.Cause{
		WatchName: watch,
		Kind:      causes.CauseExternalDelete,
		EventType: event,
...
func (hdlr ExternalResourceHandler) Update(ctx context.Context, e event.TypedUpdateEvent[client.Object], q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	oldObj := e.ObjectOld
	newObj := e.ObjectNew

	generationChanged := oldObj.GetGeneration() != newObj.GetGeneration()

	cause := causes.CauseExternalStatusOrMeta
	if generationChanged {
		cause = causes.CauseExternalSpecUpdate
	}
...
func (hdlr ExternalResourceHandler) Generic(ctx context.Context, e event.TypedGenericEvent[client.Object], q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	watch := hdlr.watchName
	...
	cause := causes.Cause{
		WatchName: watch,
		Kind:      causes.CauseExternalGeneric,
		EventType: event,
